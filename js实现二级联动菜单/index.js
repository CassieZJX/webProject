var citySelectObj = {
    ...
    citySelectNode:document.getElementsByClassName('citySelect')[0],
    ...
    showCity:function(){
        // console.log(this.citySelectNode);
        //省份的显示
        var provinceStr='';
        ...
        this.ul_province.innerHTML=provinceStr;
        this.li_province=this.ul_province.getElementsByTagName('li');//获取省份
        for(var j=0;j<this.li_province.length;j++){
            this.li_province[j].index=j;//获取当前省份的索引值
            this.li_province[j].addEventListener('mouseenter',this.CityMouseenter.bind(this));//给每个省份添加鼠标滑动事件
        }
        //bind()会返回执行上下文被改变的函数而不会立即被执行，而call,apply则是直接执行改函数
    },

   //鼠标滑过事件
    CityMouseenter:function(e){
        //避免鼠标滑过的每个省份都显示灰色，每次事件前将省份背景颜色变为白色
        for(var i=0;i<this.li_province.length;i++){
            this.li_province[i].style.backgroundColor='#fff';
        }
        var index =e.target.index;
        // console.log(index);
        var cityData=this.city[index];//获取省份下的城市
        e.target.style.backgroundColor='#f1f3f6';//触发事件的省份加上灰色背景
        this.cityNode.style.display='block';//显示当前省份下的城市
        // console.log(cityData);

         //城市的显示
        var cityStr='';//使用字符串拼接而不用append是防止DOM操作过于频繁，因为DOM操作比较慢，需要渲染引擎和js引擎
        for (var i=0;i<cityData.length;i++){
            cityStr+="<li>"+cityData[i][1]+"</li>";
        };
    ...
    showProvince:function(){
        var self =this;
        this.citySelectNode.onclick=function(){
            if(self.bool){
